...
| Framework defaults | all six, in `ops-engine` |
...
| 18 | 3 | **changed** *(superseded 14-09-2026: the nine plugins became three, and these live in `ops-engine`)* | Nothing in the plan says where framework-default capability skills live. | One new plugin, **`ops-capabilities`**, holding all five. The README already groups them as "you inherit these", so one install gets a consumer the whole inherited set; scattering them across the loop plugins would have made "what do I inherit?" unanswerable. |
